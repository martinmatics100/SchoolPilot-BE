...
{
    public static class GetUserRegularPermissions
    {
        ...
        public class Handler : IRequestHandler<Query, Result>
        {
            ...
            public async Task<Result> Handle(Query message, CancellationToken cancellationToken)
            {
                ...
                if (message.UserId.HasValue)
                {
                    ...
                    setPermissions = await (from userPrinciple in _readContext.UserPrinciples
                                            ...
                                            group permission by new { permission.Resource, permission.ResourceType, permission.Action } into g
                                            select new Model
                                            {
                                                Resource = g.Key.Resource,
                                                ResourceType = g.Key.ResourceType,
                                                Action = g.Key.Action,
                                                Value = g.Max(s => s.Value) == PermissionValue.Granted
                                            }).ToDictionaryAsync(x => new KeyModel(x.Resource, x.Action));
                }
